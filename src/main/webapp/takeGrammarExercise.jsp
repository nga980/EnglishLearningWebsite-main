...
<%@page import="model.User, model.GrammarTopic, model.QuizQuestion, java.util.List" %> <%@ taglib prefix="c" uri="jakarta.tags.core" %>
<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>Làm Bài Tập Ngữ Pháp: <c:out value="${topic.title}"/></title> <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">
...
<body>
    <jsp:include page="/common/header.jsp">
        <jsp:param name="activePage" value="grammar"/> </jsp:include>
    ...
    <div class="container mt-4">
        <div class="quiz-header">
            ...
            <h2>Bài Tập Ngữ Pháp: <c:out value="${topic.title}"/></h2> </div>
        ...
        <div class="quiz-container">
            ...
            <form method="POST" action="${pageContext.request.contextPath}/submit-grammar-exercise" id="quiz-form"> <input type="hidden" name="grammarTopicId" value="${topic.topicId}"> <c:forEach var="question" items="${questionList}" varStatus="loop">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Auto-save (lưu tạm thời trong localStorage)
            $('input[type="radio"]').on('change', function() {
                const formData = $('#quiz-form').serialize();
                // Sử dụng topic.topicId cho key trong localStorage
                localStorage.setItem('exercise_progress_' + ${topic.topicId}, formData);
            });
            
            // Khôi phục dữ liệu đã lưu (nếu có)
            const savedData = localStorage.getItem('exercise_progress_' + ${topic.topicId});
            if (savedData) {
            ...
            $('#quiz-form').on('submit', function() {
                localStorage.removeItem('exercise_progress_' + ${topic.topicId});
            });
        });
    </script>
</body>
